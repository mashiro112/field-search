...
最新通用能力审查：[新增功能 ROI 建议](reviews/2026-09-17-feature-roi.md)。优先候选为带时间戳的视频字幕和公共论坛结构化读取；运行配置与任务内续跑属于配套改善。本轮仅做源码、既有记录及上游文档审查，没有安装新能力或重跑历史验收；科研 FS03 状态保持独立。

后续授权：[R24 实施与集中验收](reviews/R24-IMPLEMENTATION.md) 已启动，按字幕、论坛、配置、批量续跑的顺序推进。当前阶段为实施中，尚未验收；上述审查的“未实施”描述保留为历史状态，当前发布源码快照尚未因派发任务而改变。
...
先读本页及当前 SOP，再按问题读取历史报告和对应源码。需要实现全貌时使用 [162 份原始源码索引](COMPLETE_SOURCE_INDEX.md)；需要演进与设计依据时使用 [28 份历史资料索引](history/INDEX.md)。代码源清单与新增文档清单分别维护，原始源码 hash 不因文档发布而改变。
