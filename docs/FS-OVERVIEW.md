...
当前最新：[R24 四项能力已安装并通过独立验收](reviews/R24-RESULTS.md)，包含 YouTube 字幕时间定位、Discourse 主题读取、本机运行配置/诊断、任务内批量复用/刷新。源码清单、索引与指令合集已同步，运行环境与会话不公开。科研 FS03 状态保持独立。

进行中：[R25 Bilibili 字幕接入](reviews/R25-BILIBILI.md)，已做旧工具与新版接口的匿名实测，尚无稳定字幕正文正例；正在准备独立扫码验证并修复初审发现的问题。当前未验收；发布源码仍为 R24 已验收基线。
