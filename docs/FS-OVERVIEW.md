...
正在执行：[R31 证据复用与时效计划](reviews/R31-EVIDENCE-REUSE-PLAN.md)。对 14 个分支做取舍，优先验证批处理中断恢复与订阅源更新检查；新功能仍待本轮验收。

最新交付：[R30 扩展探索](reviews/R30-EXPANDED-DISCOVERY.md)。比较八类能力后，新增站点入口发现和 MarkItDown 0.1.8 本地材料转换；真实站点、代表性文件、离线复用与关键失败条件已验收。正文读取现可复用配置中的既有运行环境，衔接发现的链接。当前源码清单为 182 文件；候选取舍与提取限制见结果。
